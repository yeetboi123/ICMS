%% Postprocess paired stimulation only 
...
POSTSTIM_BLANK_TIME = 0.001; % zero out output t seconds after stim
...
load('sorted_TS_current_paired.mat') % contains info about the stim trials 
...
STIM_CHANNEL_NUM = numel(unique(sorted_TS_current_paired(:,3)));
...
stim_times = sorted_TS_current_paired(:,1);

%% Create cell array that contains stim pulse timestamps for every stim channel/current combination and replicant 
pairs = unique(sorted_TS_current_paired(:,5:6),'rows');
sorted_TS_current_paired = [sorted_TS_current_paired zeros(numel(stim_times),1)];
x  = 1:numel(stim_times);
for idx = 1:size(pairs,1)
    a = x(ismember(sorted_TS_current_paired(:,5:6),pairs(idx,:),'rows'));
    sorted_TS_current_paired(a,7) = idx;
end

getCurrentInd = @(x) find(abs(-sorted_TS_current_paired(:,2) - x) < 0.001);
getChInd = @(x) find(sorted_TS_current_paired(:,7) == x);
getReplicantTS = @(stim_ch,stim_current) sorted_TS_current_paired(intersect(getCurrentInd(stim_current),getChInd(stim_ch)),1);
new_order = unique(sorted_TS_current_paired(:,5:6),'rows');

ch_pair_idx = unique(sorted_TS_current_paired(:,7)); PAIR_NUM = numel(ch_pair_idx);
nCU = 4; currents = [0.5,1,2,5]; NUM_REPLICANTS = 10;
stim_pulse_arr_paired = cell(PAIR_NUM,nCU);
for i = 1:PAIR_NUM
    for j = 1:nCU
            stim_ch = ch_pair_idx(i); stim_current = currents(j);
...
for neuron_idx = 1:NEURON_NUM
    ...
    [FIRING_DATA ,post_stim_avg_window_1_array,post_stim_avg_window_2_array] = ...
            ...
            PRE_STIM_WINDOW,POST_STIM_WINDOW,PAIR_NUM,nCU,NUM_REPLICANTS,currents,ch_pair_idx);    
    % plotting the raster plot on left-hand side
    % each row in the rasterplot is the spikes in a stimulation window
    generateRasterplot2(FIRING_DATA, channelInfo, PRE_STIM_WINDOW,POST_STIM_WINDOW,PAIR_NUM,nCU,NUM_REPLICANTS,0,ch_pair_idx);
