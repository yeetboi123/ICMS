...
image_save_path = fullfile(parent_dir, 'output_images2')
...
fs = 3e4;
A = readmda('firings.curated_combined.mda'); % read firings data
Ch = A(1,:); 
...
% Assign 1-start ind for stim pairs for easy referencing
pairs = unique(sorted_TS_current_pairedCh(:,5:6),'rows');

sorted_TS_current_pairedCh = [sorted_TS_current_pairedCh zeros(360,1)];
x  = 1:360;
for idx = 1:size(pairs,1)
    a = x(ismember(sorted_TS_current_pairedCh(:,5:6),pairs(idx,:),'rows'));
    idx
    numel(a)
    sorted_TS_current_pairedCh(a,7) = idx;
end
sorted_TS_current(a{idx},7)

getCurrentInd = @(x) find(abs(-sorted_TS_current_singleCh(:,2) - x) < 0.001);
getChInd = @(x) find(sorted_TS_current_singleCh(:,4) == x);
getReplicantTS = @(stim_ch,stim_current) sorted_TS_current_singleCh(intersect(getCurrentInd(stim_current),getChInd(stim_ch)),1);
new_order = unique(sorted_TS_current_singleCh(:,4),'rows');
...
% current, third is replicant 
nCH = STIM_CHANNEL_NUM; nCU = 6; currents1 = [0.5,1,2,5,7,10]; NUM_REPLICANTS = 10;
stim_pulse_arr_single = cell(nCH,nCU);
for i = 1:nCH
    for j = 1:nCU
            stim_ch = new_order(i); stim_current = currents1(j);
            stim_pulse_arr_single{i,j} = getReplicantTS(stim_ch,stim_current);
            
    end
end


getCurrentInd = @(x) find(abs(-sorted_TS_current_pairedCh(:,2) - x) < 0.001);
getChInd = @(x) find(sorted_TS_current_pairedCh(:,7) == x);
getReplicantTS = @(stim_ch,stim_current) sorted_TS_current_pairedCh(intersect(getCurrentInd(stim_current),getChInd(stim_ch)),1);
new_order = unique(sorted_TS_current_pairedCh(:,5:6),'rows');

ch_pair_idx = unique(sorted_TS_current_pairedCh(:,7) ); PAIR_NUM = numel(ch_pair_idx);
nCH = STIM_CHANNEL_NUM; nCU = 4; currents2 = [0.5,1,2,5]; NUM_REPLICANTS = 10;
stim_pulse_arr_paired = cell(nCH,nCU);
for i = 1:numel(ch_pair_idx)
    for j = 1:nCU
            stim_ch = ch_pair_idx(i); stim_current = currents2(j);
            stim_pulse_arr_paired{i,j} = getReplicantTS(stim_ch,stim_current);
    end
end
...
% for neuron_idx = 1
    ...
    [FIRING_DATA ,post_stim_avg_window_1_array,post_stim_avg_window_2_array] = ...
            ...
            PRE_STIM_WINDOW,POST_STIM_WINDOW,nCH,6,NUM_REPLICANTS,currents1, new_order);
        
    [FIRING_DATA2 ,post_stim_avg_window_1_array,post_stim_avg_window_2_array] = ...
            getEvents2(neuronTimestamps, stim_pulse_arr_paired, POSTSTIM_BLANK_TIME, POSTSTIM_AVG_WINDOW, ...
            PRE_STIM_WINDOW,POST_STIM_WINDOW,PAIR_NUM,4,NUM_REPLICANTS,currents2, ch_pair_idx);    
    % plotting the raster plot on left-hand side
    % each row in the rasterplot is the spikes in a stimulation window
    generateRasterplot2(FIRING_DATA, channelInfo, PRE_STIM_WINDOW,POST_STIM_WINDOW,nCH,6,NUM_REPLICANTS,0,ch_pair_idx);
    generateRasterplot2(FIRING_DATA2,channelInfo,PRE_STIM_WINDOW,POST_STIM_WINDOW,PAIR_NUM,4,NUM_REPLICANTS,720,ch_pair_idx);
...
%         subplot_tight(nCH,4,(nCH*4-3)-4*(fig-1)+2,[0.01,0.01]);
        ...
        plot((30*combinedPSTH)+140 * (fig-1),'LineWidth',1,'color',colors(7,:))
        
        
        hold on;
        gridxy(preStim_count + ((length(time_hist)-1)* [0:5]),'LineStyle',':','linewidth',0.25);
        hold on;
    ...
    for fig = (1:PAIR_NUM) 
        ...
        for curr = 1:4
            ...
            for rep = 1:NUM_REPLICANTS
                temp = [FIRING_DATA2{fig,curr}{rep,1}{1,2}];
                curr_firings = [curr_firings; temp];
            end    
        ...
        plot((30*combinedPSTH)+120 * (fig-1)+145 * (nCH-1)+25,'LineWidth',1,'color',colors(7,:))
